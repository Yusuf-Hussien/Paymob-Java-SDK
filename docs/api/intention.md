...
**Auth:** Secret Key for create/update, no auth for retrieve
...
This call uses the public retrieval endpoint with `publicKey` and `clientSecret` in the path, so no Authorization header is sent.
...
| `paymentMethods` | `List<Integer>` | Create: ✅ | List of integration IDs to offer at checkout |
| `items` | `List<Item>` | No | Line items (name, amount, quantity) |
| `billingData` | `BillingData` | ✅ | Customer billing information |
| `specialReference` | `String` | No | Your internal merchant order reference |
| `notificationUrl` | `String` | No | Webhook URL for this specific intention |
| `redirectionUrl` | `String` | No | URL to redirect customer after payment |
