...
public class PaymobExample {
    ...
    public static void main(String[] args) {
        ...
        Item item2 = new Item("Product 2", 3000, "Description 2", 2); // 60.00 EGP total
        
        IntentionRequest intentionRequest = new IntentionRequest(11000, Currency.EGP, Arrays.asList(item1, item2), billingData);
        intentionRequest.setPaymentMethods(Arrays.asList(12345, 67890)); // Integration IDs
        intentionRequest.setSpecialReference("order-12345");
        
        IntentionResponse intentionResponse = client.intentions().createIntention(intentionRequest);
        String checkoutUrl = client.intentions().getUnifiedCheckoutUrl(intentionResponse.getClientSecret());
        
        // 4. Process saved card payment
        // CIT (Customer Initiated)
        CitPaymentRequest citRequest = new CitPaymentRequest("card_token_123", "123", 10000, "EGP");
        TokenizedPaymentResponse citResponse = client.savedCards().processCitPayment(citRequest);
        
        // MIT (Merchant Initiated)
        MitPaymentRequest mitRequest = new MitPaymentRequest("card_token_123", 10000, "EGP", "order-67890");
        TokenizedPaymentResponse mitResponse = client.savedCards().processMitPayment(mitRequest);
        
        // 5. Create subscription plan
        PlanRequest planRequest = new PlanRequest("Monthly Plan", BillingCycle.MONTHLY, 5000);
        planRequest.setDescription("Monthly subscription plan");
        planRequest.setRetrialCount(3);
        SubscriptionResponse planResponse = client.subscriptions().createPlan(planRequest);
        
        // 6. Create customer subscription
        SubscriptionRequest subRequest = new SubscriptionRequest(planResponse.getPlanId(), "customer@example.com", "+201234567890");
        subRequest.setCardToken("card_token_123");
        SubscriptionResponse subResponse = client.subscriptions().createSubscription(subRequest);
        
        // 7. Transaction operations
        RefundRequest refundRequest = new RefundRequest(123456789L, 5000);
        TransactionResponse refundResponse = client.transactions().refundTransaction(refundRequest);
        
        VoidRequest voidRequest = new VoidRequest(123456789L);
        TransactionResponse voidResponse = client.transactions().voidTransaction(voidRequest);
        
        CaptureRequest captureRequest = new CaptureRequest(123456789L, 10000);
        TransactionResponse captureResponse = client.transactions().captureTransaction(captureRequest);
        
        // 8. Transaction inquiry
        InquiryResponse inquiryByMerchant = client.inquiry().byMerchantOrderId("order-12345");
        InquiryResponse inquiryByTransaction = client.inquiry().byTransactionId(123456789L);
        ...
        QuickLinkResponse linkResponse = client.quickLinks().createPaymentLink(linkRequest);
